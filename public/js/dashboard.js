...
async function pollSyncStatus() {
  ...
  if (status.running) {
    ...
    // Каждый товар сохраняется в базу сразу по ходу синхронизации (см. syncService.js),
    // поэтому таблицу можно обновлять прямо во время неё — продавец видит, как строки
    // появляются и заполняются одна за другой, а не ждёт полного завершения прогона.
    await loadProducts();
    return;
  }
