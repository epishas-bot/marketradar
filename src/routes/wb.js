...
// Построено на price_snapshots — а туда, помимо периодических проверок цены на сайте
// (priceScraper.js), теперь попадает ещё и настоящая история реально состоявшихся
// продаж с СПП от самого WB (см. syncService.js, wbStatisticsClient.js) — но это всё
// равно только статистика по фактам, а не подтверждённые площадкой причины: WB нигде не
// раскрывает, что именно определяет размер СПП в моменте.
